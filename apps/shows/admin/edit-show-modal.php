        ...
        <form class="form" id="ad-edit-show-form" name="add-edit-show-form">
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control" id="city" name="city"
                   value="<?php echo $show['city']; ?>" />

            <!-- venue -->
            <label for="venue">Show venue</label>
            <input type="text" class="form-control" id="venue" name="venue"
                   value="<?php echo $show['venue']; ?>" />

            <!-- other bands -->
            <label for="bands">Show other bands</label>
            <input type="text" class="form-control" id="bands" name="bands"
                   value="<?php echo $show['other_bands']; ?>" />

            <!-- band comments -->
            <label for="band-comments">Show band comments</label>
            <input type="text" class="form-control" id="band-comments" name="band-comments"
                   value="<?php echo $show['band_comments']; ?>" />

            <!-- setlist -->
            <label for="setlist">Show setlist</label>
            <textarea class="form-control" rows="5" id="setlist"
                      name="setlist"><?php echo $show['setlist']; ?></textarea>

            <!-- performers -->
            <label for="date">Show performers</label>
            <textarea class="form-control" rows="5" id="performers" name="performers">
              <?php echo $show['performers']; ?>
            </textarea>

          </div>

          <button type="submit" class="btn btn-primary" name="Submit" id="send-edit-show-form">
            Submit the edit
          </button>
        </form>
        <div id="added-ok" class="text-success" hidden>
          <h3>Successfully edited show! Boom...</h3>
        </div>
        <div id="add-failed" class="text-danger" hidden>
          <h3>Failed to edit show!</h3>
        </div>
      </div>
