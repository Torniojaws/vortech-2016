...
<aside class="well">
  <?php
    ...
    if ($count >= 2) {
      ...
      echo '<strong>'.$second_last_author_name[0]['name'].'</strong>: '.
            $comments[$second_last]['comment'].'<br />';
      echo '<strong>'.$last_author_name[0]['name'].'</strong>: '.
            $comments[$last]['comment'].'<br />';
  ...
  <?php
    if ($_SESSION['user_logged'] == 1) { ?>
      ...
      <form class="form-inline" role="form"
            id="user-bio-comment-<?php echo $_SESSION['user_id']; ?>">
        <div class="form-group">
          <div class="input-group">
            ...
            <input type="hidden" name="performer_id" value="<?php echo $member['id']; ?>" />
            <input type="hidden" name="modal_id" value="<?php echo $photo['id']; ?>" />
            <input type="hidden" name="user_id" value="<?php echo $_SESSION['user_id']; ?>" />
            <input type="hidden" name="username" value="<?php echo $_SESSION['display_name']; ?>" />
          </div>
        </div>
        <button type="submit" class="btn btn-default">Submit</button>
        <div id="added-ok-<?php echo $member['id']; ?>" class="text-success" hidden>
          <h3>Successfully added comment! Boom...</h3>
        </div>
        <div id="add-failed-<?php echo $member['id']; ?>" class="text-danger" hidden>
