...
<form class="form" id="user-guestbook-form" name="add-guestbook-post-form">
    ...
    <?php
      if (isset($_SESSION['user_logged']) and $_SESSION['user_logged'] == 1) {
        ...
        echo '<label for="antispam">Antispam Question:</label><br /> '.$result[0]['question'].'
              <input type="hidden" id="antispam_challenge" name="antispam_challenge" value="'.
              $random_number.'" /><br /><input type="text" id="antispam_response"
              name="antispam_response" placeholder="Your answer" /><br />';
      }
    ?>
  ...
  <button type="submit" class="btn btn-primary" name="Submit" id="send-guestbook-comment-form">
    Add the comment
  </button>
</form>
